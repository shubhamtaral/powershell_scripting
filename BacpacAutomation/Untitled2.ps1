...
function ModifyDb {
    ...
    try {
        ...
        $SQLDumpQuery = "USE $backupDBName
                        ...
                        drop security policy FilterPolicyOnMstClientType
                        drop function fnContractingParty
                        drop function fnClientType
                            "
...
# Req Variables
$Server = 'cpai-peu-sql-01.700bb8e8fb23.database.windows.net'
$DBName = 'PEU_CL_QA'
$SQLUser = 'ContractPodAi'
$SQLPasswd = 'Ol5LjGlafpDjB2cC'  
$LocalFilePath = 'F:\AutoBacpac\backups\' # (*Given path specific to mgnt specific)

# ----------------------------------------------- Direct BACPAC ----------------------------------------------- #

$DirectBacpac = $false # (Make it true if you want to take bacpac without backup & restore // false will take backup, restore it and then bacpac )

$ifConnected = $false

#LogFile
$file = $DBName + '_' + (get-date).ToString("yyyyMMdd") + ".log"
